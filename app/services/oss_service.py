...
if __name__ == "__main__":
    ...
    try:
        ...
        result = upload_file_to_oss("test_upload.txt", bucket="fci", region="cn-beijing")
        print("上传结果:")
        for key, value in result.items():
            print(f"  {key}: {value}")
        
        # 生成预签名URL（临时下载链接）
        presigned_url = generate_presigned_url(result['object_key'], 3600, bucket="fci", region="cn-beijing")
        print(f"\n临时下载链接: {presigned_url}")
        
        # 生成强制下载链接
        download_url = generate_presigned_url(
            result['object_key'], 3600, force_download=True, bucket="fci", region="cn-beijing")
        print(f"\n强制下载链接: {download_url}")
        
        # 上传文件并生成唯一名称
        result2 = upload_file_with_unique_name_to_oss("test_upload.txt", bucket="fci", region="cn-beijing")
        print("\n唯一名称上传结果:")
        for key, value in result2.items():
            print(f"  {key}: {value}")
        
        # 为唯一名称文件生成预签名URL
        presigned_url2 = generate_presigned_url(result2['object_key'], 3600, bucket="fci", region="cn-beijing")
