...
from .table_helper_new import add_html_table_to_doc
...
class BilingualDocumentGenerator:
    ...
    def add_original_text(self, text: str) -> None:
        ...
        try:
            ...
            if '<table' in text and '</table>' in text:
                # 添加原文表格，不包含翻译
                if not add_html_table_to_doc(self.document, text):
                    # 如果表格处理失败，降级为普通文本
    ...
    def add_translated_text(self, text: str) -> None:
        ...
        try:
            # 检测是否是HTML表格
            if '<table' in text and '</table>' in text:
                # 只添加翻译表格，不添加原文表格（原文表格已在add_original_text中添加）
                if not add_html_table_to_doc(self.document, text):
                    # 如果表格处理失败，降级为普通文本
                    paragraph = self.document.add_paragraph()
    ...
    def _strip_inline_markdown(text: str) -> str:
        ...
        try:
            ...
            s = re.sub(r"\*\*([^*]+)\*\*", r"", s)
            s = re.sub(r"__([^_]+)__", r"", s)
            s = re.sub(r"\*([^*]+)\*", r"", s)
            s = re.sub(r"_([^_]+)_", r"", s)
            # 行内代码: `code`
            s = re.sub(r"`([^`]+)`", r"", s)
            # 数学: $...$ / \( ... \) / \[ ... \] -> 去除包裹符，只保留内部内容
            s = re.sub(r"\$\s*([^$]+?)\s*\$", r"", s)
            ...
            s = re.sub(r"\mathsf\s*\{?\s*([^}]*)\s*\}?", r"", s)  # 处理 \mathsf{L} 等格式
            s = re.sub(r"\mathrm\s*\{?\s*([^}]*)\s*\}?", r"", s)  # 处理 \mathrm{R} 等格式
            # 处理更复杂的带空格花括号情况
...
')
        ...
        while i < len(lines):
            ...
            j = i + 1
            while j < len(lines):
                nxt = (lines[j] or "").rstrip('')
                nxt_stripped = nxt.lstrip()
                if not nxt.strip():
                    break
                if nxt_stripped.startswith('#') or nxt_stripped.startswith('![') or                    nxt_stripped.startswith('* ') or nxt_stripped.startswith('- '):
                    break
